...
export interface ArraySchema<ChildSchema extends StandardSchemaV1> extends StandardSchemaV1<GetSchemaInput<ChildSchema>[]> {
    type: "array";
    message: string;
    childSchema: ChildSchema;
}

function pipeResults<T extends { issues: readonly StandardSchemaV1.Issue[] } | { value: any; issues: undefined }>(results: T[]): StandardSchemaV1.Result<
    (T extends { value: any } ? T["value"] : never)[]
> {
 ...
 */
export function array<ChildSchema extends StandardSchemaV1>(childSchema: ChildSchema, message = "Not a valid array"): ArraySchema<ChildSchema> {
...
export interface TupleSchema<ChildSchemas extends readonly StandardSchemaV1[]> extends StandardSchemaV1<RemapSchemas<ChildSchemas>> {
    type: "tuple";
    message: string;
 ...
 */
export function tuple<ChildSchemas extends readonly StandardSchemaV1[]>(childSchemas: ChildSchemas, message = "Not a valid tuple"): TupleSchema<ChildSchemas> {
    return createStandardSchema<TupleSchema<ChildSchemas>>("tuple", message, {
        childSchemas,
    }, (input) => {
        if (!Array.isArray(input)) {
            return {
